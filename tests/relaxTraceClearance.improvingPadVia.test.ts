...
test("projection moves an existing pad via toward clearance without changing its transitions", (): void => {
  ...
  const input: RepairRegionInput = {
    srj: { layerCount: 2, minTraceWidth: 0.1, bounds,
      obstacles: [{ type: "rect", center: { x: 0, y: 0 }, width: 0.2, height: 0.2,
        layers: ["top"], connectedTo: ["owner"] }],
      connections: [{ name: "owner", pointsToConnect: [] }, { name: "foreign", pointsToConnect: [] }] },
    routes: [{
      connectionName: "owner", traceThickness: 0.1, viaDiameter: 0.3,
      route: [{ x: -1, y: -1, z: 0 }, { x: -0.2, y: 0, z: 0 },
        { x: -0.2, y: 0, z: 1 }, { x: 1, y: -1, z: 1 }], vias: [{ x: -0.2, y: 0 }],
    }, {
      connectionName: "foreign", traceThickness: 0.1, viaDiameter: 0.3,
      route: [{ x: 0.05, y: -1, z: 0 }, { x: 0.05, y: 1, z: 0 }], vias: [],
    }], bounds, boundaryMargin: 0.2,
  ...
  expect(output[0]!.route.map((point) => point.z)).toEqual(input.routes[0]!.route.map((point) => point.z))
