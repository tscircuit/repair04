...
test("fixed-obstacle rejection preserves proposal accounting without evaluating indexed DRC", (): void => {
  ...
  const internal = solver as unknown as {
    generateCandidates: () => Generator<{
      routeIndex: number
      route: Repair04SolverInput["routes"][number]
    }>
  ...
  internal.generateCandidates = function* (): ReturnType<
    typeof internal.generateCandidates
  > {
    yield proposal
  }
  ...
  const existingContact: Repair04SolverInput = {
    ...
    srj: {
      ...input.srj,
      obstacles: [{ ...input.srj.obstacles[0]!, layers: ["top", "bottom"] }],
    },
    routes: [
      {
        ...input.routes[0]!,
  ...
  const worse = {
    routeIndex: 0,
    route: {
      ...existingContact.routes[0]!,
      route: [endpoints[0]!, { x: 0, y: 0, z: 0 }, endpoints[1]!],
    },
    evaluatedScore: undefined as unknown,
  }
  const clear = {
  ...
  access.generateCandidates = function* (): ReturnType<
    typeof internal.generateCandidates
  > {
    yield worse
    yield clear
