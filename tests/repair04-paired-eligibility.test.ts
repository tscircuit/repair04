...
test("paired search requires an indexed same-layer crossing between the exact two routes", (): void => {
  ...
  wrongId.score.errors = wrongId.score.errors.map((error): Record<string, unknown> => ({ ...error, pcb_trace_error_id: "overlap_repair04_0_repair04_9" }))
  ...
  wrongLayerInput.routes[1]!.route = wrongLayerInput.routes[1]!.route.map((point) => ({ ...point, z: 1 }))
