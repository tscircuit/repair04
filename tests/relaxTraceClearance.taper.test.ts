...
test("clearance projection retains taper widths and the fixed pad contacts", (): void => {
  ...
  expect(
    output.route.map((point) => (point as RepairRoutePoint).traceThickness),
  ).toEqual(
    before.route.map((point) => (point as RepairRoutePoint).traceThickness),
  )
  ...
  for (let i = 1; i < output.route.length; i++) {
    const a = output.route[i - 1]! as RepairRoutePoint,
      b = output.route[i]! as RepairRoutePoint
    const clearance =
      segmentToBoundsMinDistance(a, b, bounds) -
      Math.max(a.traceThickness!, b.traceThickness!) / 2
