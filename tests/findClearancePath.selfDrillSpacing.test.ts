...
test("incumbent and searched paths keep distinct holes apart within the same route", (): void => {
  ...
  const barriers: HighDensityRoute[] = [0, 2].map(
    (z): HighDensityRoute => ({
      connectionName: `barrier${z}`,
      traceThickness: 0.05,
      viaDiameter: 0.3,
      route: [
        { x: z === 0 ? 0.2 : -0.2, y: -1, z },
  ...
  for (const existingPath of [undefined, signal.route]) {
    ...
    const output = findClearancePath({
      srj,
      routes: [signal, ...barriers],
      routeIndex: 0,
      bounds,
      start: signal.route[0]!,
      end: signal.route.at(-1)!,
      traceThickness: 0.1,
      traceClearance: 0.1,
      viaClearance: 0.1,
    ...
    const holes = output!.filter(
      (point, index): boolean => index > 0 && point.z !== output![index - 1]!.z,
    )
