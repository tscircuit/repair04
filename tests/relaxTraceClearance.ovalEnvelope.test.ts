...
test("projection leaves clearance around the enclosing corners of an oval pad", (): void => {
  ...
  const input: RepairRegionInput = {
    srj: {
      ...
      obstacles: [{ type: "oval", center: { x: 0, y: 0 }, width: 0.2, height: 0.2,
    ...
    routes: [{
      ...
      route: [{ x: -1, y: 1, z: 0 }, { x: 0.1, y: 0.27, z: 0 },
        { x: 0.27, y: 0.1, z: 0 }, { x: 1, y: -1, z: 0 }],
    }],
    bounds, boundaryMargin: 0.1, lockedPointIndices: [[true, false, false, true]],
  }
  const padBounds = { minX: -0.1, maxX: 0.1, minY: -0.1, maxY: 0.1 }
  expect(segmentToBoundsMinDistance(input.routes[0]!.route[1]!, input.routes[0]!.route[2]!, padBounds) - 0.05).toBeLessThan(0.1)
