...
type PhysicalVia = Omit<
  ReturnType<typeof getRepairViaGeometry>[number],
  "pointIndices"
>
...
test("atomic three-route restoration clears a rectangle while retaining terminal tails and vias", (): void => {
    ...
    obstacles: [
      {
        ...
        height: 0.3,
        layers: ["top", "bottom"],
        connectedTo: ["pcb_plated_hole_fixed"],
      },
    ],
  }
  const routes: HighDensityRoute[] = [-0.5, 0, 0.5].map(
    (y, i): HighDensityRoute => ({
      connectionName: `route${i}`,
      traceThickness: 0.1,
      viaDiameter: 0.3,
      vias: [
        { x: -2, y },
  ...
  let nodes = 0,
    calls = 0
  const candidate = generateThreeRouteCandidates({
    srj,
    routes,
    bounds: { minX: -4.5, minY: -4.5, maxX: 4.5, maxY: 4.5 },
    violations,
  ...
  for (const replacement of candidate!)
    next[replacement.routeIndex] = replacement.route
  ...
  for (let i = 0; i < routes.length; i++) {
    ...
    expect(
      getRepairViaGeometry(next[i]!, srj.layerCount).map(
        ({ pointIndices, ...via }): PhysicalVia => via,
      ),
    ).toEqual(
      getRepairViaGeometry(routes[i]!, srj.layerCount).map(
        ({ pointIndices, ...via }): PhysicalVia => via,
      ),
    )
    expect(next[i]!.route.every((p): boolean => p.z === 0 || p.z === 1)).toBe(
      true,
    )
  }
  const solver = new Repair04Solver({
    ...
    boundaryMargin: 0.5,
    lockedPointIndices: next.map((r): boolean[] =>
      r.route.map((_p, i): boolean => i === 0 || i === r.route.length - 1),
    ),
    maxCandidateAttempts: 1,
  })
  const score = (
    solver as unknown as {
      evaluate(routes: HighDensityRoute[]): { count: number }
    }
  ).evaluate(next)
