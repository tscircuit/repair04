...
import { Repair04Solver, extractRepairRegion, getFixedObstacleViolations, getNewViaPadViolations, getRepairViaGeometry, mergeRepairRegion } from "../lib"
...
test("a pad-contact via moves directly to a legal envelope face without rerouting its terminals", (): void => {
  ...
  const srj: SimpleRouteJson = {
    ...
    obstacles: [
      { type: "rect", center: { x: 0, y: 0.3 }, width: 0.8, height: 0.2, layers: ["top"], connectedTo: ["signal", "end"] },
      { type: "rect", center: { x: 0, y: -0.2 }, width: 0.8, height: 0.2, layers: ["top"], connectedTo: ["foreign"] },
    ],
  }
  const region = extractRepairRegion({ srj, routes: [route], bounds: srj.bounds })
  const before = structuredClone(region)
  const solver = new Repair04Solver({ ...region, allowLayerChanges: true, maxCandidates: 4, maxPathSearchNodes: 1000 })
  solver.solve()
  ...
  expect(getNewViaPadViolations({ srj, previousRoutes: region.routes.map((route): HighDensityRoute => ({ ...route, route: [], vias: [] })), routes: output })).toEqual([])
  expect(getFixedObstacleViolations({ srj, routes: output })).toEqual([])
  const merged = mergeRepairRegion({ routes: [route], region, repairedRoutes: output })[0]!
  ...
  expect(getRepairViaGeometry(merged, 2).map((via): number[] => via.layerSequence)).toEqual([[1, 0]])
