...
test("a tapered polyline moves as a group with actual pad clearance above0.1mm", (): void => {
  ...
  const bounds = { minX: pad.center.x - pad.width / 2, maxX: pad.center.x + pad.width / 2, minY: pad.center.y - pad.height / 2, maxY: pad.center.y + pad.height / 2 }
  ...
  for (let i = 1; i < output.route.length; i++) {
    ...
    minimumGap = Math.min(minimumGap, segmentToBoundsMinDistance(a, b, bounds) - width / 2)
    expect(b.traceThickness).toBe((original.route[i]! as RepairRoutePoint).traceThickness)
  }
