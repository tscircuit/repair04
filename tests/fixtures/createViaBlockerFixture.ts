...
export const createViaBlockerFixture = (
...
): { solver: any; selected: { routeIndex: number; viaIndex: number }; candidate: any } => {
  const owner: HighDensityRoute = options.owner ?? {
    connectionName: "owner", rootConnectionName: "owner",
    traceThickness: 0.1, viaDiameter: 0.3, vias: [{ x: 0, y: 0 }],
    route: [
      { x: 0, y: -2, z: 0 }, { x: 0, y: 0, z: 0 },
      { x: 0, y: 0, z: 1 }, { x: 2, y: 0, z: 1 },
    ],
  }
  const routes: HighDensityRoute[] = [owner, ...[-0.2, 0.3].map(
    (x, index): HighDensityRoute => ({
      connectionName: `foreign${index}`, rootConnectionName: `foreign${index}`,
      traceThickness: 0.1, viaDiameter: 0.3, vias: [],
      route: [{ x, y: -1, z: 0 }, { x, y: 1, z: 0 }],
    }),
  )]
  const bounds = { minX: -5, maxX: 5, minY: -5, maxY: 5 }
  const srj: SimpleRouteJson = {
    layerCount: options.layerCount ?? 2, minTraceWidth: 0.1,
    bounds, connections: [], obstacles: [], traces: options.fixedTraces ?? [],
  }
  const selected = { routeIndex: 0, viaIndex: 0 }
  const solver: any = new Repair04Solver({
    srj, bounds, boundaryMargin: 0.5, routes,
    lockedPointIndices: routes.map((route): boolean[] =>
      route.route.map((_, index): boolean =>
        index === 0 || index === route.route.length - 1,
      ),
    ),
    allowLayerChanges: false, movableVias: [selected],
    maxCandidates: 128, maxCandidateAttempts: options.maxAttempts ?? 128,
  ...
  const candidate = {
    ...
    route: {
      ...moved,
      vias: geometry.filter((item, index): boolean =>
        geometry.findIndex((other): boolean =>
