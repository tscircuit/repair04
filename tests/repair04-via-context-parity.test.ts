...
type Access = {
  ...
  getIndexedViaIds(
    ...
    via: ReturnType<typeof getRepairViaGeometry>[number],
  ): Set<string>
}
type Counter = {
  generation: number
...
test("selected-via context is rebuilt after acceptance and shared only within one owner sweep", (): void => {
  const routes: HighDensityRoute[] = [-2, 2].flatMap(
    (x, group): HighDensityRoute[] => [
      {
        connectionName: `owner${group}`,
        traceThickness: 0.1,
        viaDiameter: 0.3,
        vias: [{ x, y: 0 }],
        route: [
  ...
  const input: Repair04SolverInput = {
    ...
    routes,
    lockedPointIndices: routes.map((r): boolean[] =>
      r.route.map((_, i): boolean => i === 0 || i === r.route.length - 1),
    ),
    movableVias: [
      { routeIndex: 0, viaIndex: 0 },
      { routeIndex: 3, viaIndex: 0 },
    ],
  ...
  const before = structuredClone(input),
    variant = new Repair04Solver(input)
  ...
  expect(
    new Set(
      [...newCount.oldKeys.keys()].map((key): string => key.split(":")[1]!),
    ).size,
  ).toBe(2)
  expect([...newCount.oldKeys.values()].every((n): boolean => n === 1)).toBe(
    true,
  )
