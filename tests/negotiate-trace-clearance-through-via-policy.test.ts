...
test("negotiated clearance accounts for through-via copper outside routed layers", (): void => {
    ...
    for (const allowBlindAndBuriedVias of [undefined, false, true]) {
      ...
      const result = negotiateTraceClearance({
        ...
        isLocked: (routeIndex, pointIndex): boolean =>
          routeIndex === 0 ||
          pointIndex === 0 ||
          pointIndex === routes[routeIndex]!.route.length - 1,
