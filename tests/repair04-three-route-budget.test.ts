...
test("three-route searches retain lifetime call and shared node limits after restart", (): void => {
      ...
      obstacles: [
        {
          ...
          height: 0.3,
          layers: ["top", "bottom"],
          connectedTo: ["pcb_plated_hole_test"],
        },
      ],
    },
    routes: [-0.6, 0, 0.6].map(
  ...
  const access = solver as unknown as {
    score: unknown
    evaluate(routes: typeof input.routes): unknown
    generateCandidatesForMode(mode: boolean): Generator<unknown>
    threeRoutePathSearchCalls: number
    threeRoutePathSearchNodes: number
