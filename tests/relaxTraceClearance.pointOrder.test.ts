...
test("whole-board projection retains redundant vertices and original via transition indices", (): void => {
  ...
  const input: RepairRegionInput = {
    srj: { layerCount: 2, minTraceWidth: 0.1, bounds, obstacles: [], connections: [] },
    routes: [{ connectionName: "signal", traceThickness: 0.1, viaDiameter: 0.3,
      route: [{ x: -2, y: 0, z: 0 }, { x: -0.5, y: 0, z: 0 },
        { x: -0.5, y: 0, z: 0 }, { x: 0.5, y: 0, z: 0 },
        { x: 0.5, y: 0, z: 1 }, { x: 0.5, y: 0, z: 1 }, { x: 2, y: 0, z: 1 }],
      vias: [{ x: 0.5, y: 0 }] },
    { connectionName: "neighbor", traceThickness: 0.1, viaDiameter: 0.3,
      route: [{ x: -2, y: 0.25, z: 0 }, { x: -0.5, y: 0.16, z: 0 },
        { x: 0.5, y: 0.16, z: 0 }, { x: 2, y: 0.25, z: 0 }], vias: [] }],
    bounds, boundaryMargin: 0,
  ...
  for (let ri = 0; ri < input.routes.length; ri++) {
    const before = input.routes[ri]!, after = output[ri]!
    expect(after.route.length).toBe(before.route.length)
    expect(after.route.map((point) => point.z)).toEqual(before.route.map((point) => point.z))
  ...
  expect(output[0]!.vias).toEqual([{ x: output[0]!.route[4]!.x, y: output[0]!.route[4]!.y }])
