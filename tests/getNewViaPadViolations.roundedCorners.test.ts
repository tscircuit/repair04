...
test("via and fixed-copper guards measure actual rounded pads including rotated capsules", (): void => {
  for (const sample of [
    ...
    const route: HighDensityRoute = {
      ...
      route: [{ x, y, z: 0 }, { x, y, z: 1 }], vias: [{ x, y }],
    }
    const srj: SimpleRouteJson = {
      bounds: { minX: -5, maxX: 5, minY: -5, maxY: 5 }, layerCount: 2, minTraceWidth: 0.1,
      connections: [{ name: "signal", pointsToConnect: [] }],
      obstacles: [{ type: "oval", center: { x: 0, y: 0 }, width: sample.width, height: sample.height,
        ccwRotationDegrees: sample.rotation, layers: ["top"], connectedTo: ["foreign"] }],
    }
    const previousRoutes: HighDensityRoute[] = [{ ...route, route: [], vias: [] }]
    expect(getNewViaPadViolations({ srj, previousRoutes, routes: [route] })).toEqual([])
    expect(getFixedObstacleViolations({ srj, routes: [route] })).toEqual([])
    srj.obstacles[0]!.type = "rect"
    expect(getNewViaPadViolations({ srj, previousRoutes, routes: [route] }).length).toBeGreaterThan(0)
    expect(getFixedObstacleViolations({ srj, routes: [route] }).length).toBeGreaterThan(0)
  }
})
