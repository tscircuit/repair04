...
test("tapered translations preserve locked contacts, ports, junctions and vias", (): void => {
  for (const kind of ["lock", "port", "junction", "via", "endpoint"]) {
    ...
    if (kind === "junction") Object.assign(route.route[1]!, { portPointId: "fixed-junction" })
    if (kind === "via") {
    ...
    const candidates = [...(solver as unknown as Access).generateTaperedSegmentCandidates([{ ri: 0, pi, distance: 0, t: 0.5 }])]
