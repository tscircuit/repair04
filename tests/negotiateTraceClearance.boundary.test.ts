...
test("negotiation preserves border copper and cannot shortcut through the fixed collar", (): void => {
  ...
  const routes: HighDensityRoute[] = [{ connectionName: "signal", traceThickness: 0.1,
    viaDiameter: 0.3, vias: [], route: [
      { x: -6, y: -4.5, z: 0 }, { x: -3, y: -4.5, z: 0 },
      { x: 0, y: -3, z: 0 }, { x: 3, y: -4.5, z: 0 }, { x: 6, y: -4.5, z: 0 },
    ] }]
  const region = extractRepairRegion({ srj, routes, bounds, boundaryMargin: 0.5 })
  const result = negotiateTraceClearance({ srj: region.srj, routes: region.routes,
    bounds: region.mutableBounds, dirtyRouteIndices: [0],
  ...
  const merged = mergeRepairRegion({ routes, region, repairedRoutes: result.routes })
