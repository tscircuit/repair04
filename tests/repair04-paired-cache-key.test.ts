...
type Access = { generateCandidates(): Generator<Candidate>; evaluate(routes: HighDensityRoute[]): unknown }
...
test("candidate memoization distinguishes changes to only the third route", (): void => {
  ...
  const solver = new Repair04Solver(input), access = solver as unknown as Access
  ...
  access.generateCandidates = function* (): Generator<Candidate> {
    for (const offset of [0.1, 0.1, 0.2]) {
      const partner = { ...input.routes[2]!, route: input.routes[2]!.route.map(
        (point, index): HighDensityRoute["route"][number] => index === 1 || index === 2 ? { ...point, y: point.y + offset } : point,
      ) }
      yield { routeIndex: 0, route: input.routes[0]!, additionalRoutes: [{ routeIndex: 1, route: input.routes[1]! }, { routeIndex: 2, route: partner }] }
    }
  }
  solver.solve()
