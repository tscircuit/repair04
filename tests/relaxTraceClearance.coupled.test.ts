...
test("coupled gap projection retains anchors and opens real trace and via clearances", (): void => {
  ...
  expect(
    new AutoroutingDrcEngine(input.srj).evaluate(
      convertRepairRoutesToTraces(routes, 2),
    ).errors.length,
  ).toBeGreaterThan(0)
  expect(
    new AutoroutingDrcEngine(input.srj).evaluate(
    ...
    for (let j = i + 1; j < output.length; j++) {
      for (let a = 1; a < output[i]!.route.length; a++) {
        for (let b = 1; b < output[j]!.route.length; b++) {
          const p = output[i]!.route[a - 1]!,
            q = output[i]!.route[a]!
          const r = output[j]!.route[b - 1]!,
            s = output[j]!.route[b]!
          if (
            Math.max(p.z, q.z) < Math.min(r.z, s.z) ||
            Math.max(r.z, s.z) < Math.min(p.z, q.z)
          )
            continue
          const radius =
            (p.z === q.z ? 0.05 : 0.15) + (r.z === s.z ? 0.05 : 0.15)
          expect(
            segmentToSegmentMinDistance(p, q, r, s) - radius,
