...
> **Benchmark correction:** the original wrapper omitted dedicated via-in-pad and via-to-pad checks. Its reported passing-board totals are withdrawn. The complete corrected V10 run passes 4/15 current boards and 6/37 older boards, up from zero; the requested +30% improvement is not established. PR #2420 remains in draft.
...
The original 5/15 and 8/37 passing totals used incomplete via-pad coverage and are withdrawn. The complete corrected V10 benchmark uses solver commit `fc78a05d4c65cb41dc38214bbeed5f4d5657055d` and autorouter commit `0e9cd6f8f4d31a09439948fed6e7f0155290ad8c`, identical expanded checks on both sides, and every board in each published dataset revision. It changes the current 15-board revision from 0 to 4 passing boards (+26.67 percentage points) and the older 37-board revision from 0 to 6 (+16.22 points). Three candidate runs time out (sample003, sample045, sample055), and all remain in both denominators. A relative increase is undefined from a zero-pass baseline, and a +30-percentage-point improvement is not established.

V10's full bugreport94 test, whose post-repair03 geometry and replay context match SRJ33 sample054, passes its limit of five default DRC errors and the targeted-overlap assertions in actual Linux CI after 571.97 seconds, with a reviewed snapshot change. The macOS test passes those assertions in 255.28 seconds with snapshot update mode enabled. These assertions do not require zero expanded DRC errors. CI owns the 1,200-second test limit. Delivery CI is still pending.

A subsequent local-scoring correction makes unrestricted layer repair aware of existing unlocked via-pad violations. It has a regression test and a separately verified two-region sample050 proof with full DRC zero. That focused proof does not alter the complete V10 counts; actual pipeline replay and a new complete benchmark are required to assess the correction.
...
The [benchmark report and saved-output verification](https://github.com/tscircuit/tscircuit-autorouter/blob/codex/repair04-bounded-drc/docs/benchmarks/repair04/repair04-srj33-report.md) record corrected results and provenance. Historical V8 evidence is explicitly superseded.
